...
public class ModBlocks
{
	...
	public static final RegistryObject<Block> SOUL_POWDER_BLOCK = REGISTRY.register("soul_powder_block", () -> new Block(AbstractBlock.Properties.of(Material.SAND, MaterialColor.COLOR_CYAN).strength(0.5F).lightLevel((state) -> 5).sound(SoundType.SAND).harvestTool(ToolType.SHOVEL)));
	...
	public static final RegistryObject<Block> REINFORCED_GLASS = REGISTRY.register("reinforced_glass", () -> new ReinforcedGlassBlock(AbstractBlock.Properties.of(Material.GLASS).strength(5.0F, 3600000.0F).sound(SoundType.GLASS).harvestTool(ToolType.PICKAXE).noOcclusion().isValidSpawn(ModBlocks::never).isRedstoneConductor(ModBlocks::never).isSuffocating(ModBlocks::never).isViewBlocking(ModBlocks::never)));
