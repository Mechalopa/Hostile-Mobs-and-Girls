			...
			if (biomeKey != null)
			{
				...
				if ((BiomeDictionary.hasType(biomeKey, BiomeDictionary.Type.NETHER) && !isVoid) || ModUtils.checkList(event.getName(), ModConfigs.cachedServer.SPAWN_NETHER_BIOME_WHITELIST))
				{
					if (biomeKey == Biomes.SOUL_SAND_VALLEY)
					{
						...
						{
							event.getSpawns().getSpawner(MobCategory.MONSTER).add(new SpawnerData(ModEntityTypes.GHSATLY_SHEEKER.get(), ModConfigs.cachedServer.GHSATLY_SHEEKER_SPAWN_WEIGHT_IN_SOUL_SAND_VALLEY, 1, 1));
					...
					else if (biomeKey == Biomes.BASALT_DELTAS)
					{
						...
						if (ModConfigs.cachedServer.GHSATLY_SHEEKER_SPAWN_WEIGHT_IN_BASALT_DELTAS > 0)
							event.getSpawns().getSpawner(MobCategory.MONSTER).add(new SpawnerData(ModEntityTypes.GHSATLY_SHEEKER.get(), ModConfigs.cachedServer.GHSATLY_SHEEKER_SPAWN_WEIGHT_IN_BASALT_DELTAS, 1, 1));
					...
					else if (biomeKey != Biomes.CRIMSON_FOREST && biomeKey != Biomes.WARPED_FOREST)
					{
						...
						if (ModConfigs.cachedServer.GHSATLY_SHEEKER_SPAWN_WEIGHT > 0)
							event.getSpawns().getSpawner(MobCategory.MONSTER).add(new SpawnerData(ModEntityTypes.GHSATLY_SHEEKER.get(), ModConfigs.cachedServer.GHSATLY_SHEEKER_SPAWN_WEIGHT, 1, 1));
