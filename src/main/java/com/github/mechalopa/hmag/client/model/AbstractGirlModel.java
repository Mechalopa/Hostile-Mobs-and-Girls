...
@OnlyIn(Dist.CLIENT)
public abstract class AbstractGirlModel<T extends LivingEntity> extends BipedModel<T>
{
	protected ModelRenderer bodyPart1;
	protected ModelRenderer bodyPart2;
	protected ModelRenderer bust;
	...
	public AbstractGirlModel(float modelSize, float yOffsetIn, int textureWidthIn, int textureHeightIn, boolean isArmor, boolean hasBust)
	{
		...
		if (!isArmor)
		{
			...
			{
				this.body = new ModelRenderer(this, 16, 16);
				this.body.addBox(-3.0F, 0.0F, -1.5F, 6.0F, (float)this.getBodyHeight() + 5.0F, 3.0F, modelSize);
